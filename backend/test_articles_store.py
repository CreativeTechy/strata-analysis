...
class ListIdeaClustersForProjectTests(unittest.TestCase):
    def test_falsy_project_id_returns_empty_page_without_querying(self):
        with patch("articles_store.db.fetch_all") as mock_fetch_all:
    ...
    def test_no_database_configured_returns_empty_page(self):
        with patch("articles_store.config.DATABASE_URL", ""):
    ...
    def test_returns_rows_and_total_from_the_query(self):
        ...
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_all", return_value=rows) as mock_fetch_all:
                with patch("articles_store.db.fetch_one", return_value={"total": 7}):
    ...
    def test_query_error_returns_empty_page_instead_of_raising(self):
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_all", side_effect=RuntimeError("boom")):
                result = articles_store.list_idea_clusters_for_project(1)
        self.assertEqual(result["clusters"], [])
...
class ListArticlesForIdeaClusterTests(unittest.TestCase):
    ...
    def test_cluster_not_in_project_returns_none(self):
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_one", return_value=None):
                self.assertIsNone(articles_store.list_articles_for_idea_cluster(1, 2))

    def test_returns_articles_page_when_cluster_belongs_to_project(self):
        article_rows = [{"id": 10, "title": "EV Review"}]
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_one", side_effect=[{"id": 1}, {"total": 1}]):
                with patch("articles_store.db.fetch_all", return_value=article_rows):
    ...
    def test_query_error_returns_none(self):
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_one", side_effect=RuntimeError("boom")):
                self.assertIsNone(articles_store.list_articles_for_idea_cluster(1, 2))
...
class ListAnalysisErrorsTests(unittest.TestCase):
    ...
    def test_filters_to_failed_status(self):
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_all", return_value=[]) as mock_fetch_all:
                with patch("articles_store.db.fetch_one", return_value={"total": 0}):
...
class GetArticleAnalysisTests(unittest.TestCase):
    ...
    def test_article_not_found_returns_none(self):
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_all", return_value=[]):
                with patch("articles_store.db.fetch_one", return_value=None):
    ...
    def test_shapes_a_successful_analysis(self):
        ...
        with patch("articles_store.config.DATABASE_URL", "postgresql://x"):
            with patch("articles_store.db.fetch_all", return_value=[{"column_name": k} for k in row]):
                with patch("articles_store.db.fetch_one", return_value=row):
