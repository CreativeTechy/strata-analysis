...
def _extract_document(document: dict, disk_path: Path, filename: str) -> None:
    ...
    db.execute("delete from competitor_document_chunks where document_id = %s", (document_id,))
    ...
    for chunk in competitor_document_extraction.iter_chunks(disk_path, filename):
        db.execute(
            ...
            values (%s, %s, %s, %s, %s)
            """,
            (document_id, chunk["index"], chunk["text"] or None, chunk["method"], chunk["error"]),
        )
        if chunk["error"]:
    ...
    db.execute(
        ...
        update competitor_documents
         ...
         where id = %s
        """,
        (combined_text or None, method_summary, " | ".join(errors) or None, status, document_id),
    )
    ...
    if status != "processed":
        ...
        db.execute("update competitor_documents set articles_status = 'skipped' where id = %s", (document_id,))
        return
...
def _process_record_document(document: dict, disk_path: Path, filename: str) -> None:
    ...
    document_id = int(document["id"])
    db.execute(
        "update competitor_documents set status = 'processing', total_chunks = 1, processed_chunks = 0 where id = %s",
        (document_id,),
    )
    db.execute("delete from competitor_document_chunks where document_id = %s", (document_id,))
    ...
    db.execute("update competitor_documents set articles_status = 'generating' where id = %s", (document_id,))
    try:
        competitor_document_articles.generate_candidates_from_records(
