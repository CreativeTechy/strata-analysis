...
class ResolveManyTests(unittest.TestCase):
    ...
    def test_operator_override_wins_over_the_seeded_default(self):
        ...
        override_row = {
            "source_key": "real:reuters.com", "tier": "untrusted", "reason": "Known to syndicate unverified claims.",
            "set_by_name": "alice", "updated_at": "2026-01-01T00:00:00Z",
        }
...
class SetTierTests(unittest.TestCase):
    ...
    def test_records_an_audit_row_before_upserting_the_current_state(self):
        user = {"id": 7, "username": "alice"}
        with patch("services.articles.source_trust.db.execute") as mock_execute, \
             patch("services.articles.source_trust.db.fetch_one", return_value={"tier": "trusted"}) as mock_fetch_one:
            result = source_trust.set_tier("real:reuters.com", "real", "trusted", "Wire service.", user)

        mock_execute.assert_called_once()
        insert_sql, insert_params = mock_execute.call_args[0]
        self.assertIn("source_trust_reviews", insert_sql)
        self.assertEqual(insert_params, ("real:reuters.com", "real", "trusted", "Wire service.", 7, "alice"))

        mock_fetch_one.assert_called_once()
        upsert_sql, _ = mock_fetch_one.call_args[0]
