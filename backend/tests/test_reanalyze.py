...
class ReanalyzeArticleTests(unittest.TestCase):
    ...
    def test_article_not_found_reports_not_found_without_running_analysis(self):
        with patch("reanalyze.load_article_for_reanalysis", return_value=None), \
             patch("reanalyze.analyze_article") as mock_analyze:
    ...
    def test_successful_analysis_saves_and_reports_ok(self):
        ...
        with patch("reanalyze.load_article_for_reanalysis", return_value=dict(self.ARTICLE_ROW)), \
             patch("reanalyze._primary_project_id_for_article", return_value=5), \
             patch("reanalyze.analyze_article", return_value=analysis_result), \
             patch("reanalyze.save_articles", return_value=(1, {})) as mock_save:
    ...
    def test_failed_analysis_status_is_not_ok_even_though_save_succeeded(self):
        ...
        with patch("reanalyze.load_article_for_reanalysis", return_value=dict(self.ARTICLE_ROW)), \
             patch("reanalyze._primary_project_id_for_article", return_value=None), \
             patch("reanalyze.analyze_article", return_value=analysis_result), \
             patch("reanalyze.save_articles", return_value=(1, {})):
            result = reanalyze.reanalyze_article(1)
        self.assertFalse(result["ok"])
    ...
    def test_analyze_article_raising_marks_failed_and_returns_ok_false(self):
        with patch("reanalyze.load_article_for_reanalysis", return_value=dict(self.ARTICLE_ROW)), \
             patch("reanalyze._primary_project_id_for_article", return_value=None), \
             patch("reanalyze.analyze_article", side_effect=RuntimeError("boom")), \
             patch("reanalyze._mark_failed") as mock_mark_failed, \
             patch("reanalyze.save_articles") as mock_save:
            result = reanalyze.reanalyze_article(1)
        self.assertFalse(result["ok"])
        self.assertEqual(result["analysis_status"], "failed")
    ...
    def test_save_failure_is_reported_without_raising(self):
        ...
        with patch("reanalyze.load_article_for_reanalysis", return_value=dict(self.ARTICLE_ROW)), \
             patch("reanalyze._primary_project_id_for_article", return_value=None), \
             patch("reanalyze.analyze_article", return_value=analysis_result), \
             patch("reanalyze.save_articles", side_effect=RuntimeError("db down")), \
             patch("reanalyze._mark_failed") as mock_mark_failed:
            result = reanalyze.reanalyze_article(1)
        self.assertFalse(result["ok"])
        self.assertEqual(result["analysis_status"], "failed")
...
class ReanalyzeArticlesBatchTests(unittest.TestCase):
    def test_runs_each_article_independently_and_collects_results(self):
        with patch("reanalyze.reanalyze_article", side_effect=lambda aid: {"article_id": aid, "ok": True}) as mock_single:
...
class MarkHelpersTests(unittest.TestCase):
    def test_mark_processing_swallows_db_errors(self):
        with patch("reanalyze.db.execute", side_effect=RuntimeError("boom")):
