...
-- =============================================================================
alter table public.articles add column if not exists source_run_snapshot jsonb;
alter table public.articles add column if not exists source_provenance jsonb;

create table if not exists public.evidence_run_articles (
    run_id text not null references public.pipeline_runs(id) on delete cascade,
    project_id bigint not null references public.projects(id) on delete cascade,
    article_id bigint not null references public.articles(id) on delete cascade,
    content_hash text, source_snapshot jsonb,
    captured_at timestamptz not null default now(),
    primary key (run_id, article_id)
);
create index if not exists evidence_run_articles_project_idx on public.evidence_run_articles (project_id, run_id);
...
create table if not exists public.evidence_run_status (
    ...
    article_count integer not null default 0, claim_count integer not null default 0,
...
create index if not exists evidence_run_status_project_idx on public.evidence_run_status (project_id, updated_at desc);
...
create table if not exists public.evidence_claims (
    ...
    fingerprint text not null, claim_text text not null,
    claim_type text not null default 'factual_assertion', topic text not null default 'General',
    entities jsonb not null default '[]'::jsonb, geography text, time_scope text,
    dates jsonb not null default '[]'::jsonb, quantities jsonb not null default '[]'::jsonb,
    assessment text not null default 'insufficient_evidence', explanation text, limitations text,
    processing_status text not null default 'success', processing_error text, model text,
    rules_version text not null default 'evidence-v1',
    supporting_count integer not null default 0, contradicting_count integer not null default 0,
    contextual_count integer not null default 0, distinct_origins integer not null default 0,
    created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
    constraint evidence_claims_type_check check (claim_type in ('factual_assertion','attributed_statement','forecast','opinion','causal_explanation')),
    constraint evidence_claims_assessment_check check (assessment in ('supported','contradicted','mixed_evidence','insufficient_evidence','not_yet_verifiable','assessment_unavailable')),
    constraint evidence_claims_processing_check check (processing_status in ('pending','success','failed')),
    constraint evidence_claims_run_fingerprint_key unique (run_id, fingerprint)
);
create index if not exists evidence_claims_filter_idx on public.evidence_claims (project_id, run_id, topic, assessment);

create table if not exists public.evidence_items (
    id bigint generated always as identity primary key,
    claim_id bigint not null references public.evidence_claims(id) on delete cascade,
    article_id bigint not null,
    relationship text not null, passage text not null, citation_valid boolean not null default false,
    origin_key text, source_snapshot jsonb, created_at timestamptz not null default now(),
    constraint evidence_items_relationship_check check (relationship in ('supporting','contradicting','contextual')),
...
create table if not exists public.evidence_reviews (
    ...
    reviewer_name text, decision text not null, reason text not null,
    created_at timestamptz not null default now(),
    constraint evidence_reviews_decision_check check (decision in ('supported','contradicted','mixed_evidence','insufficient_evidence','not_yet_verifiable'))
);
create index if not exists evidence_reviews_claim_idx on public.evidence_reviews (claim_id, created_at desc);
...
create index if not exists evidence_provenance_reviews_article_idx on public.evidence_provenance_reviews (project_id, article_id, created_at desc);

drop trigger if exists set_evidence_claims_updated_at on public.evidence_claims;
create trigger set_evidence_claims_updated_at before update on public.evidence_claims
for each row execute function public.set_updated_at();

drop trigger if exists set_evidence_run_status_updated_at on public.evidence_run_status;
create trigger set_evidence_run_status_updated_at before update on public.evidence_run_status
for each row execute function public.set_updated_at();

-- Evidence accuracy fields must exist in the baseline before generation indexes below.
...
alter table public.evidence_run_articles drop constraint if exists evidence_run_articles_analysis_source_check;
alter table public.evidence_run_articles add constraint evidence_run_articles_analysis_source_check
...
alter table public.evidence_run_status add column if not exists generation integer not null default 0;
alter table public.evidence_reviews drop constraint if exists evidence_reviews_decision_check;
alter table public.evidence_reviews add constraint evidence_reviews_decision_check check
...
create table if not exists public.evidence_assessment_revisions (
    id bigint generated always as identity primary key,
    claim_id bigint not null references public.evidence_claims(id) on delete cascade,
    generation integer not null, assessment text not null, explanation text,
    supporting_count integer not null default 0, contradicting_count integer not null default 0,
    contextual_count integer not null default 0, distinct_origins integer not null default 0,
...
-- Project-aware evidence relevance and atomic generation publication.
alter table public.evidence_claims
...
create table if not exists public.evidence_generations (
    ...
    project_id bigint not null references public.projects(id) on delete cascade,
    status text not null default 'pending', scope_snapshot jsonb not null default '{}'::jsonb,
    scope_hash text not null, rules_version text not null, model text,
    candidate_count integer not null default 0, classified_count integer not null default 0,
    direct_count integer not null default 0, contextual_count integer not null default 0,
    unrelated_count integer not null default 0, uncertain_count integer not null default 0,
    error text, started_at timestamptz, finished_at timestamptz, published_at timestamptz,
    created_at timestamptz not null default now(), primary key (run_id, generation),
...
create table if not exists public.evidence_article_screenings (
    run_id text not null references public.pipeline_runs(id) on delete cascade,
    generation integer not null, project_id bigint not null references public.projects(id) on delete cascade,
    article_id bigint not null references public.articles(id) on delete cascade,
    content_hash text not null, duplicate_key text not null, decision text not null,
    quality_code text not null, reason text not null, best_passage text, passage_score numeric,
    relevance text not null, decision_method text not null, model text,
    decision_config jsonb not null default '{}'::jsonb, rules_version text not null,
    created_at timestamptz not null default now(), primary key (run_id,generation,article_id),
    constraint evidence_article_screenings_decision_check check (decision in ('included','excluded','needs_review')),
    constraint evidence_article_screenings_relevance_check check (relevance in ('direct','contextual','unrelated','uncertain'))
);
create index if not exists evidence_article_screenings_review_idx on public.evidence_article_screenings (project_id,run_id,generation,decision,quality_code);
create table if not exists public.evidence_article_screening_reviews (
    id bigint generated always as identity primary key, project_id bigint not null references public.projects(id) on delete cascade,
    run_id text not null references public.pipeline_runs(id) on delete cascade,
    article_id bigint not null references public.articles(id) on delete cascade,
    reviewer_id bigint references public.users(id) on delete set null, reviewer_name text,
    decision text not null, reason text not null, created_at timestamptz not null default now(),
    constraint evidence_article_screening_reviews_decision_check check (decision in ('include','exclude','needs_review'))
);
create index if not exists evidence_article_screening_reviews_lookup_idx on public.evidence_article_screening_reviews (project_id,run_id,article_id,created_at desc);
create table if not exists public.evidence_claim_candidates (
    run_id text not null references public.pipeline_runs(id) on delete cascade,
    generation integer not null, project_id bigint not null references public.projects(id) on delete cascade,
    article_id bigint not null references public.articles(id) on delete cascade,
    fingerprint text not null, topic text not null, claim_text text not null, passage text,
    status text not null, reason text not null, decision_method text not null, rules_version text not null,
    created_at timestamptz not null default now(), primary key (run_id,generation,article_id,fingerprint),
    constraint evidence_claim_candidates_status_check check (status in ('accepted','rejected','needs_review'))
);
create index if not exists evidence_claim_candidates_review_idx on public.evidence_claim_candidates (project_id,run_id,generation,status);
create table if not exists public.evidence_passage_relevance_cache (
    scope_hash text not null, content_hash text not null, rules_version text not null, model text not null,
    decision_config_hash text not null, relevance text not null, score numeric, passage text, reason text not null,
    updated_at timestamptz not null default now(),
    primary key (scope_hash,content_hash,rules_version,model,decision_config_hash),
    constraint evidence_passage_relevance_cache_label_check check (relevance in ('direct','contextual','unrelated','uncertain'))
);
create table if not exists public.evidence_relevance_cache (
    scope_hash text not null, fingerprint text not null, content_hash text not null default '',
    rules_version text not null, model text not null default '', relevance text not null,
    explanation text, score numeric, processing_status text not null default 'success',
    updated_at timestamptz not null default now(),
...
create table if not exists public.evidence_relevance_reviews (
    ...
    project_id bigint not null references public.projects(id) on delete cascade,
    run_id text not null references public.pipeline_runs(id) on delete cascade,
    fingerprint text not null, reviewer_id bigint references public.users(id) on delete set null,
    reviewer_name text, decision text not null, reason text not null,
    created_at timestamptz not null default now(),
