...
-- Per-document breakdown for a single run. A document is this product's unit
-- of provenance, so it answers the same question the crawler's per-source
...
create index if not exists pipeline_run_documents_run_idx on public.pipeline_run_documents (run_id);
...
drop policy if exists "Public read access" on public.pipeline_run_documents;
create policy "Public read access"
on public.pipeline_run_documents
for select
to anon, authenticated
using (true);
