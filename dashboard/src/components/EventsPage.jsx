...
import { CalendarDays, Plus, Pencil, Trash2, Check, X, Search, Flag, Clock3, Layers3, Link2 } from 'lucide-react';
...
export default function EventsPage({
  ...
  const visibleEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      const matchesQuery =
        !needle ||
        [event.name, event.status, event.start_date, event.end_date, ...feedNames]
          .filter(Boolean)
