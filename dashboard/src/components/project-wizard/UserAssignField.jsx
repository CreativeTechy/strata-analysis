      ...
      <div className="assign-sources-header">
        <div>
          <div className="assign-sources-kicker">
            <Users size={12} style={{ verticalAlign: -1, marginRight: 4 }} /> {t('userAssign.kicker')}
          </div>
          <strong className="assign-sources-title">{t('userAssign.title')}</strong>
        </div>
        <div className="assign-sources-summary">
          <span className="panel-chip">{t('userAssign.selectedCount', { count: selectedIds.length })}</span>
        </div>
      </div>
      ...
      <div className="assign-sources-list">
        {users.length === 0 ? (
          ...
          <div className="admin-empty-state" style={{ padding: '16px 10px' }}>
            ...
            <strong>{t('userAssign.noMatchesTitle')}</strong>
            <span>{t('userAssign.noMatchesBody')}</span>
          </div>
        ) : (
            ...
            return (
              <label key={user.id} className={`assign-source-item ${isSelected ? 'selected' : ''}`}>
                ...
                <div className="assign-source-copy">
                  ...
                  <div className="assign-source-url">{user.email || t('userAssign.noEmail')}</div>
                </div>
              </label>
            );
          })
